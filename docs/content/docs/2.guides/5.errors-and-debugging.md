...
- replaces the document with a **safe error panel** that shows the code, template, and relative
...
- **`/_pdf` returns 404** in a production build. The preview routes exist only
  in development.
- **`PDF_LAYOUT_ERROR: Font family not registered`** means that the `fontFamily` in a
  style is not declared in `pdf.fonts`, or the face (weight/style) is missing.
- **`PDF_ASSET_INVALID`** means that the image or font path escapes its root, has the
  wrong signature, or exceeds its size cap.
- **`PDF_LIMIT_EXCEEDED`** means that the document's layout depends on the page numbers it
