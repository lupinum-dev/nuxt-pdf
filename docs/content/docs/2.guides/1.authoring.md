...
Real documents repeat the same recipes: an eyebrow label, a footer, a table
header. Keep them in one module and import it where you need it. A theme
module under `pdfs/components/` travels with the components that use it:
...
Use spread overrides for one-off variations instead of restating every key:

```vue
<PdfText :style="{ ...eyebrow, color: '#8A2D2B' }">
  Overdue
</PdfText>
```

When two files must agree on geometry, such as a table header and its body
rows, put the column widths in one shared module and read them from both
sides. The playground invoice shows this pattern in
