...
the multi-pass loop and add no cost.
...
`id → page` map back in, until it stabilizes.
...
A document whose *layout* depends on the numbers it prints — a TOC entry whose
height changes with its own page number — never converges. After `maxPasses`
...
ancestor's. Set `pageMode="useOutlines"` on `PdfDocument` to open the reader with
the outline panel showing:

```vue
<PdfDocument page-mode="useOutlines">
  <PdfPage><PdfText>Report</PdfText></PdfPage>
</PdfDocument>
```

The outline carries title text and parent/child nesting. Destination geometry
(`top`/`left`/`zoom`/`fit`) and click actions beyond navigation are not claimed.
...
and a fixed footer with a dynamic page counter — all resolved through the
multi-pass loop.
