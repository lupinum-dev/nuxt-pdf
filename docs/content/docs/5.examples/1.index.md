...
description: Six worked showcase templates in the playground — invoice, report, annual report, certificate, e-book, and menu — and the technique each one demonstrates.
...
the same registry in development and production, and covered by a rendering test
with a reviewed raster baseline. Run `pnpm dev` and open `/_pdf` to browse them,
or read each source alongside the excerpts below.

::callout{icon="lucide:info"}
The render test pipeline compiles a single SFC; it does not transform nested
child `.vue` imports. Templates that want helpers keep them as plain `.ts`
modules under `pdfs/components/**` (models, geometry, render-function
components) and inline their markup — the same shape as the shipped tests.
::
...
| Certificate | `pdfs/certificate.vue` | SVG ornament craft — an engraver's border and a seal |
...
## E-book — chapter-aware running matter
...
## Annual report — SVG charts and Views tables

The Fieldnote Studio annual report draws a grouped bar chart, a line chart with a
gradient area tint, and a donut — all from the [SVG primitives]($docs/guides/svg-graphics),
...
The financial statement has no first-class table primitive — it is built from
...
## Certificate — SVG ornament craft
...
concentric seal — two rules, a 32-dot beaded coin edge, a five-point star, and a
single bronze `RadialGradient` used once.

```ts [pdfs/components/certificate/CertificateSeal.ts]
...
at spaces to two centred lines and the page can never spill to a second sheet.

## Menu — pure typography and diacritics

The Gasthaus Alpenrose card is two A5 pages of typography only — no borders, no
...
Prices are formatted for the locale — `9.5` becomes `9,50` — and the whole card
