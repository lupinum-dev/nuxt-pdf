---
title: Registry and render result
description: "The typed #pdf server module, runtime dispatch, and completed render conversions."
icon: lucide:braces
---
...
The handle does not expose the compile-time definition, preview data, source
file, or a second preview render method.
...
Diagnostics contain no document content, props, or resource URLs. An
application can send them to its metrics layer.
...
Editing a template rebuilds its registry and refreshes the document with its
diagnostics. The preview keeps the selected scenario.
