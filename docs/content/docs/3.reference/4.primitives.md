...
| `ownerPassword`, `userPassword` | `string` | Encryption passwords — untested passthrough, see below |
| `permissions` | `object` | Printing/modifying/copying/… permission flags — untested passthrough, see below |

::callout{icon="lucide:triangle-alert"}
`ownerPassword`, `userPassword`, and `permissions` are typed passthroughs to the
underlying engine with **no conformance coverage** — unlike the rest of this
table, no fixture verifies them. Validate the encryption behaviour yourself
