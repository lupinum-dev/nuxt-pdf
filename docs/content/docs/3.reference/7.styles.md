...
| Colour | A colour string such as `'#18251d'`, `'rgb(24, 37, 29)'`, or `'red'` | There are no CSS variables or global cascade. |
...
| -------- | -------------- | --------- | --------------------------- |
| `backgroundColor` | colour string | Only through nested `PdfText` runs | Border/background raster conformance; otherwise use on page-flow primitives. |
| `color` | colour string | Yes | Text inheritance conformance; applies to text-bearing primitives. |
...
| `borderColor`, `borderTopColor`, `borderRightColor`, `borderBottomColor`, `borderLeftColor` | colour string | No | Page-flow primitives; border painting is raster-compared. |
...
syntax. Use the explicit width, style, and colour keys so invalid values remain
type-checkable.
...
| `textDecoration` | `'none' \| 'underline' \| 'line-through'` or both decoration names separated by a space | Yes | Every value renders in paired React/Vue raster parity; underline colour/style also has a reviewed baseline. |
| `textDecorationColor` | colour string | No | `PdfText` and `PdfLink`. |
