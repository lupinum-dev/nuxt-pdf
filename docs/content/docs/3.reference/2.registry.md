...
`result.metadata` is the frozen `title`, `filename`, and `language` written into
the completed PDF for this render: `definePdf` values win when present, otherwise
authored `PdfDocument` fallbacks. `result.diagnostics` is one frozen object shared
with the development preview:
